...
if not app.secret_key:
    raise ValueError(
        "FLASK_SECRET_KEY belum diatur pada file .env"
    )
...
def on_mqtt_message(client, userdata, msg):
    ...
    try:
        ...
        if (
            suhu > SUHU_ATAS
            and
            not alert_state.suhu_tinggi
        ):

            msg_text = (
                f"PERINGATAN SUHU TINGGI\n"
                f"Suhu ruang server "
                f"mencapai {suhu:.1f} °C"
            )
            ...
            suhu <= SUHU_ATAS
            and
            alert_state.suhu_tinggi
        ):

            msg_text = (
                f"Suhu kembali normal: "
                f"{suhu:.1f} °C"
            )
        ...
        if (
            ...
            suhu >= SUHU_BAWAH
            and
            alert_state.suhu_rendah
        ):

            msg_text = (
                f"Suhu dingin kembali "
                f"normal: {suhu:.1f} °C"
            )
        ...
        if (
            lembab > LEMBAB_ATAS
            and
            not alert_state.lembab_tinggi
        ):

            msg_text = (
                f"KELEMBAPAN TINGGI\n"
                f"Kelembapan mencapai "
                f"{lembab:.0f}%"
            )
            ...
            lembab <= LEMBAB_ATAS
            and
            alert_state.lembab_tinggi
        ):

            msg_text = (
                f"Kelembapan kembali "
                f"normal: {lembab:.0f}%"
            )
        ...
        if amper < AMPER_BAWAH:
            ...
            if not alert_state.arus_mati:
                ...
                msg_text = (
                    ...
                    f"Arus terdeteksi: "
                    f"{amper:.2f} A\n"
                    f"Server akan shutdown "
                    f"otomatis dalam 5 menit."
                )
                ...
                if (
                    durasi_mati >=
                    DURASI_MAKS_ARUS_MATI
                ):

                    msg_shutdown = (
                        "UPS HABIS\n"
                        "Sistem melakukan "
                        "shutdown server otomatis."
                    )
        ...
        # =========================================================
            ...
            if (
                alert_state.arus_mati
                or
                alert_state.daya_overload
            ):
